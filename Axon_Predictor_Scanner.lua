-- PSICOSENATICO | AXON PREDICTOR SCANNER - SOURCE TRACE
-- Passive / zero-hook.
-- Captures only Roblox Instances, UI properties, attributes and ValueBases.
-- Does NOT hook functions/remotes, use debug/getgc, or intercept HTTP.
...
local state = {base=nil, predictor=nil, refresh=nil, status=nil, gui=nil}
...
local function safeProp(x,p)
    local ok,r = pcall(function() return x[p] end)
    if not ok then return nil end
    local t = typeof(r)
    if t=="string" or t=="number" or t=="boolean" then return r end
    return tostring(r)
end
...
local function captureObject(x, force)
    local p=pathOf(x)
    local attrs=safeAttributes(x)
    local text=nil
    if x:IsA("TextLabel") or x:IsA("TextButton") or x:IsA("TextBox") then
        text=safeProp(x,"Text")
    end

    local keep = force or interesting(p) or interesting(x.Name) or (text and interesting(text))
    if not keep then
    ...
    local item={class=x.ClassName,name=tostring(x.Name),path=p,attributes=attrs}
    if text ~= nil then item.text=tostring(text); item.visible=safeProp(x,"Visible") end
...
local function detectAxonRoots()
    local roots={}
...
local function captureUI()
    local out={}
    local axonRoots=detectAxonRoots()
...
local function captureReplicated()
    local out={}
    ...
    for _,x in ipairs(list) do
        if x:IsA("ValueBase") or x:IsA("Folder") or x:IsA("Configuration")
            ...
            if keep then
                local item={class=x.ClassName,name=tostring(x.Name),attributes=attrs}
                if x:IsA("ValueBase") then item.value=safeProp(x,"Value") end
                out[p]=item
            end
        end
...
local function snapshot(label)
    return {
        label=label, unix=os.time(), clock=os.clock(),
        placeId=tostring(game.PlaceId), gameId=tostring(game.GameId),
        jobId=tostring(game.JobId), ui=captureUI(), replicated=captureReplicated()
    }
end
...
local function diffMap(a,b)
    ...
    for k,v in pairs(b) do
        ...
        elseif stable(a[k])~=stable(v) then changed[k]={before=a[k],after=v} end
    end
...
local function report()
    return {
        meta={version="AxonPredictorSourceTrace",zeroHook=true,passive=true,created=os.time(),
              note="UI/attributes/ValueBases/replicated metadata only"},
        base=state.base,predictor=state.predictor,refresh=state.refresh,
...
local function captureBase()
    ...
    task.defer(function()
        state.base=snapshot("base"); state.predictor=nil; state.refresh=nil
        setStatus("BASE OK. Agora carregue Axon e abra Egg Predictor.")
    end)
end
...
local function capturePredictor()
    ...
    task.defer(function()
        ...
        setStatus("PREDICTOR OK. Pressione Refresh Predictions no Axon.")
    end)
end
...
local function captureRefresh()
    ...
    task.defer(function()
        ...
        setStatus("REFRESH OK. Agora EXPORTAR.")
    end)
end

local function exportData()
    if not state.base then setStatus("Nada para exportar.") return end
    local ok,json=pcall(function() return encode(report(),{}) end)
    if not ok then setStatus("Erro export: "..tostring(json)) return end
...
sg.Name="PSICO_AXON_SOURCE_TRACE"; sg.ResetOnSpawn=false; sg.DisplayOrder=1400; sg.Parent=CoreGui
state.gui=sg

local frame=Instance.new("Frame")
frame.AnchorPoint=Vector2.new(.5,.5); frame.Position=UDim2.fromScale(.5,.5)
frame.Size=UDim2.fromOffset(470,285); frame.BackgroundColor3=Color3.fromRGB(9,18,34)
frame.BorderSizePixel=0; frame.Parent=sg
Instance.new("UICorner",frame).CornerRadius=UDim.new(0,14)

local title=Instance.new("TextLabel")
title.BackgroundTransparency=1; title.Position=UDim2.fromOffset(14,10)
title.Size=UDim2.new(1,-28,0,28); title.Text="AXON PREDICTOR - SOURCE TRACE - ZERO-HOOK"
title.Font=Enum.Font.GothamBold; title.TextSize=13; title.TextColor3=Color3.fromRGB(238,245,255)
title.TextXAlignment=Enum.TextXAlignment.Left; title.Parent=frame

local status=Instance.new("TextLabel")
status.Position=UDim2.fromOffset(14,48); status.Size=UDim2.new(1,-28,0,78)
status.BackgroundColor3=Color3.fromRGB(15,29,52); status.BorderSizePixel=0
status.Text="Pronto. Comece com CAPTURAR BASE."; status.Font=Enum.Font.Code; status.TextSize=11
status.TextColor3=Color3.fromRGB(215,229,247); status.TextWrapped=true; status.Parent=frame
Instance.new("UICorner",status).CornerRadius=UDim.new(0,9); state.status=status

local function button(text,x,y,w)
    local b=Instance.new("TextButton")
    b.Position=UDim2.fromOffset(x,y); b.Size=UDim2.fromOffset(w,38)
    b.BackgroundColor3=Color3.fromRGB(31,78,132); b.BorderSizePixel=0; b.Text=text
    b.TextColor3=Color3.fromRGB(244,248,255); b.Font=Enum.Font.GothamBold; b.TextSize=11
    b.Parent=frame; Instance.new("UICorner",b).CornerRadius=UDim.new(0,9)
    return b
end
