--[[
PSICOSENATICO | Steal An Egg - Inventory Egg ESP V4
Independent inventory ESP page + independent filters.
- Third sidebar tab under FILTROS ESP.
- Separate enable/disable + separate rarity/$s/value/pet/mutation/display options.
- Does not reuse map ESP filters.
- Draws in a dedicated ScreenGui while Roblox Backpack Inventory is open.
- Uses slot AbsolutePosition/AbsoluteSize; does not depend on ToolName.Visible.
]]
...
local State={
    Alive=true,Connections={},CatalogIndex={},CatalogEntries={},InventoryEggs={},Drawn={},
    LastSave=0,LastRecognized=0,LastVisible=0,LastDrawn=0
}
...
local function mkLabel(parent,text,pos,size,ts)
    local l=Instance.new("TextLabel"); l.BackgroundTransparency=1; l.BorderSizePixel=0; l.Position=pos; l.Size=size; l.Font=Enum.Font.Gotham; l.Text=text; l.TextColor3=Color3.fromRGB(230,235,246); l.TextSize=ts or 9; l.TextXAlignment=Enum.TextXAlignment.Left; l.Parent=parent; return l
end
local function mkButton(parent,text,pos,size)
    local b=Instance.new("TextButton"); b.BackgroundColor3=Color3.fromRGB(35,44,61); b.BorderSizePixel=0; b.Position=pos; b.Size=size; b.Font=Enum.Font.GothamMedium; b.Text=text; b.TextColor3=Color3.fromRGB(242,246,255); b.TextSize=10; b.AutoButtonColor=true; b.Parent=parent; round(b,8); return b
end
local function mkBox(parent,placeholder,pos,size)
    local b=Instance.new("TextBox"); b.BackgroundColor3=Color3.fromRGB(29,38,55); b.BorderSizePixel=0; b.Position=pos; b.Size=size; b.Font=Enum.Font.Gotham; b.Text=""; b.PlaceholderText=placeholder; b.TextColor3=Color3.fromRGB(242,246,255); b.PlaceholderColor3=Color3.fromRGB(128,145,176); b.TextSize=9; b.ClearTextOnFocus=false; b.Parent=parent; round(b,8); return b
end
...
local function indexEntry(key,cfg)
    ...
    for _,name in ipairs({e.Key,e.PetName,e.EggDisplayName}) do
        if type(name)=="string" and name~="" then State.CatalogIndex[normalize(name)]=e; State.CatalogIndex[normalize(name:gsub("%s+[Ee][Gg][Gg]$",""))]=e end
    end
...
local function copyMutations(record)
    ...
    if typeof(src)=="table" then for _,m in pairs(src) do add(m) end end; add(record.BaseMutation or (item and item.BaseMutation)); if record.HasParasite==true or (item and item.HasParasite==true) then add("Parasite") end
...
local function buildBuckets()
    local buckets={}
    local function add(k,e) k=normalize(k or ""); if k=="" then return end; buckets[k]=buckets[k] or {}; buckets[k][#buckets[k]+1]=e end
...
local function infoText(egg)
    ...
    local second={}
    if CFG.ShowEarnings then second[#second+1]=egg.EarningsPerSecond and ("$"..formatCompact(egg.EarningsPerSecond).."/s") or "$?/s" end
    if CFG.ShowEggValue then second[#second+1]=egg.SellPrice and ("$"..formatCompact(egg.SellPrice)) or "$?" end
    if CFG.ShowWeight then second[#second+1]=egg.WeightLabel or (egg.WeightKg and (formatCompact(egg.WeightKg).."kg") or "?kg") end
...
local function clearDrawn()
    for _,rec in pairs(State.Drawn) do if rec.Frame then pcall(function() rec.Frame:Destroy() end) end end; State.Drawn={}; State.LastDrawn=0
end
...
local function drawSlot(slot,egg)
    ensureOverlay(); local pos,size=slot.AbsolutePosition,slot.AbsoluteSize; if size.X<20 or size.Y<20 then return false end
    ...
    if not rec or not rec.Frame.Parent then
        ...
        local stroke=Instance.new("UIStroke"); stroke.Name="Border"; stroke.Thickness=2; stroke.Transparency=.05; stroke.Parent=f
        local badge=Instance.new("TextLabel"); badge.Name="Badge"; badge.BackgroundColor3=Color3.fromRGB(5,8,15); badge.BackgroundTransparency=.18; badge.BorderSizePixel=0; badge.Font=Enum.Font.GothamBold; badge.TextWrapped=true; badge.TextScaled=true; badge.TextStrokeColor3=Color3.new(0,0,0); badge.TextStrokeTransparency=.08; badge.ZIndex=101; badge.Parent=f; round(badge,5)
    ...
    rec.Frame.Position=UDim2.fromOffset(pos.X,pos.Y); rec.Frame.Size=UDim2.fromOffset(size.X,size.Y); rec.Stroke.Color=color
    local a,b=infoText(egg); rec.Badge.Text=b and (a.."\n"..b) or a; rec.Badge.TextColor3=color
    local h=b and math.clamp(math.floor(size.Y*.28),28,40) or math.clamp(math.floor(size.Y*.18),20,28)
    rec.Badge.Position=UDim2.fromOffset(3,3); rec.Badge.Size=UDim2.new(1,-6,0,h); rec.Frame.Visible=true; return true
end
...
local function getInventoryParts()
    local rg=CoreGui:FindFirstChild("RobloxGui"); local backpack=rg and rg:FindFirstChild("Backpack"); local inv=backpack and backpack:FindFirstChild("Inventory")
    local scroll=inv and inv:FindFirstChild("ScrollingFrame",true); return backpack,inv,scroll
end
local function insideRect(slot,scroll)
    if not (slot and scroll) then return false end
    local p,s=slot.AbsolutePosition,slot.AbsoluteSize; local rp,rs=scroll.AbsolutePosition,scroll.AbsoluteSize
    local cx,cy=p.X+s.X*.5,p.Y+s.Y*.5
    return cx>=rp.X and cx<=rp.X+rs.X and cy>=rp.Y and cy<=rp.Y+rs.Y
end
local function updateStatus()
    if statusText then statusText.Text=string.format("Inv:%d • Slots:%d • Vis:%d • Draw:%d",State.LastSave,State.LastRecognized,State.LastVisible,State.LastDrawn) end
end
local function refreshESP()
    if not State.Alive then return end
    if not CFG.Enabled then clearDrawn(); updateStatus(); return end
    refreshInventoryData(); local _,inv,scroll=getInventoryParts()
    if not (inv and scroll and inv.Visible) then clearDrawn(); State.LastVisible=0; State.LastDrawn=0; updateStatus(); return end
    ...
    for _,d in ipairs(scroll:GetDescendants()) do
        if d:IsA("TextLabel") and d.Name=="ToolName" and d.Text~="" then
            local name=cleanSlotName(d.Text); local cfg=findCatalog(name)
            if cfg then
                recognized=recognized+1; local slot=d.Parent
                if slot and slot:IsA("GuiObject") and insideRect(slot,scroll) then
...
local function locateBaseUi()
    ...
    mainPage=normal and normal.Parent or nil; filterPage=rarity and rarity.Parent or nil
    contentHost=mainPage and mainPage.Parent or (filterPage and filterPage.Parent); sidebar=tabFilters and tabFilters.Parent or nil
...
local function makeCycleButton(parent,labelText,y,values,getter,setter)
    ...
    connect(b.MouseButton1Click,function()
        local cur=getter(); local idx=1; for i,v in ipairs(values) do if v==cur then idx=i break end end; idx=idx%#values+1; setter(values[idx]); b.Text=getter(); refreshESP()
    end)
    return b
end
local function makeInlineToggle(parent,text,y,key)
    ...
    connect(b.MouseButton1Click,function() CFG[key]=not CFG[key]; b.Text=CFG[key] and "ON" or "OFF"; refreshESP() end); return b
end

local function buildIndependentPage()
    tabInv=mkButton(sidebar,"ESP INVENTÁRIO",UDim2.fromOffset(0,100),UDim2.new(1,0,0,42))
    invPage=Instance.new("ScrollingFrame"); invPage.Name="InventoryESPPage"; invPage.BackgroundTransparency=1; invPage.BorderSizePixel=0; invPage.Position=UDim2.fromOffset(8,8); invPage.Size=UDim2.new(1,-16,1,-16); invPage.ScrollBarThickness=3; invPage.ScrollBarImageColor3=Color3.fromRGB(94,139,223); invPage.Visible=false; invPage.Parent=contentHost

    local y=0
    mkLabel(invPage,"Ativar ESP do inventário",UDim2.fromOffset(0,y),UDim2.new(.45,0,0,30),9)
    local enable=mkButton(invPage,"OFF",UDim2.new(.47,0,0,y),UDim2.new(.53,-4,0,30)); y=y+36
    local function paintEnable() enable.Text=CFG.Enabled and "ON" or "OFF"; enable.BackgroundColor3=CFG.Enabled and Color3.fromRGB(42,91,190) or Color3.fromRGB(35,44,61) end
    paintEnable(); connect(enable.MouseButton1Click,function() CFG.Enabled=not CFG.Enabled; paintEnable(); refreshESP() end)
    ...
    makeInlineToggle(invPage,"Mostrar peso",y,"ShowWeight"); y=y+36
    ...
    local reset=mkButton(invPage,"Limpar filtros do inventário",UDim2.fromOffset(0,y),UDim2.new(1,-4,0,30)); y=y+36
    invPage.CanvasSize=UDim2.fromOffset(0,y)

    local function syncBoxes() CFG.MinEarnings=parseSmartNumber(earnings.Text); CFG.MinSellPrice=parseSmartNumber(value.Text); CFG.PetFilter=pet.Text or ""; refreshESP() end
    connect(earnings.FocusLost,syncBoxes); connect(value.FocusLost,syncBoxes); connect(pet.FocusLost,syncBoxes)
    connect(reset.MouseButton1Click,function()
        CFG.MinRarity=0; CFG.MinEarnings=0; CFG.MinSellPrice=0; CFG.PetFilter=""; CFG.MutationMode="Todas"; CFG.ShowEarnings=true; CFG.ShowEggValue=false; CFG.ShowWeight=false
        earnings.Text=""; value.Text=""; pet.Text=""; refreshESP()
    end)
    ...
    connect(tabInv.MouseButton1Click,function()
        mainPage.Visible=false; filterPage.Visible=false; invPage.Visible=true
        tabMain.BackgroundColor3=Color3.fromRGB(35,44,61); tabFilters.BackgroundColor3=Color3.fromRGB(35,44,61); tabInv.BackgroundColor3=Color3.fromRGB(42,91,190)
    end)
end

local function updateVersion()
    for _,d in ipairs(mainFrame:GetDescendants()) do if d:IsA("TextLabel") and d.Text:find("V8.3",1,true)==1 then d.Text="V8.4.3 • INVENTORY ESP V4"; return end end
end
local function cleanup()
    if not State.Alive then return end; State.Alive=false; clearDrawn()
    if overlayGui then pcall(function() overlayGui:Destroy() end) end; if invPage then pcall(function() invPage:Destroy() end) end; if tabInv then pcall(function() tabInv:Destroy() end) end
    for _,c in ipairs(State.Connections) do disconnect(c) end; _G.PSICO_INVENTORY_ESP_CLEANUP=nil
end
...
if not locateBaseUi() then warn("[PSICO Inventory ESP V4] base UI não localizada"); cleanup(); return end
if not resolveModules() then warn("[PSICO Inventory ESP V4] Data.Assets não encontrado"); cleanup(); return end
buildCatalog(); buildIndependentPage(); updateVersion(); refreshInventoryData(); updateStatus()
connect(baseGui.AncestryChanged,function(_,p) if p==nil then cleanup() end end)

task.defer(function()
    while State.Alive do
        local _,inv=getInventoryParts()
        if CFG.Enabled and inv and inv.Visible then refreshESP(); task.wait(.18) else if CFG.Enabled then clearDrawn(); State.LastVisible=0; State.LastDrawn=0; updateStatus() end; task.wait(.55) end
    end
end)
