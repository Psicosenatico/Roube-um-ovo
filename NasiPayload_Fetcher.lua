--[[
PSICOSENATICO | Nasi Payload Fetcher V3

FETCH-ONLY research utility.
- Never executes downloaded payloads.
- No loadstring on remote bodies.
- No hooks / decompile / remote mutation.
- Does not alter eggs, luck, cycle, speed or combat.

Primary target in V3:
https://raw.githubusercontent.com/JualNasiRendang/omak/main/121.lua
]]
...
local TARGETS={
    {
        name="omak_121_current",
        label="OMAK 121",
        url="https://raw.githubusercontent.com/JualNasiRendang/omak/main/121.lua",
    },
    {
...
local function analyze(body)
    ...
    if #body<=300000 then
        ...
        sample=(body:sub(1,150000).."\n"..body:sub(-150000)):lower()
    end

    local keys={
        "predict","prediction","egg","spawn","upcoming","hours","ahead",
        "rarity","divine","eternal","secret","mythic","cosmic",
        "periodindex","daystartsat","rarespawns","schedule","cycle","seed","random",
        "serverhop","server hop","assetlottery","fieldeggraritiesshown",
        "askfieldeggrarityshows","eggstate","areaeggcycle","areaeggresetcycle"
    }
    for _,k in ipairs(keys) do
        if sample:find(k,1,true) then out.keywords[#out.keywords+1]=k end
    end

    out.luraph=sample:find("luraph",1,true)~=nil
    out.looksLua=sample:find("loadstring",1,true)~=nil
        or sample:find("local ",1,true)~=nil
        or sample:find("return",1,true)~=nil
    out.cloudflare=sample:find("cloudflare",1,true)~=nil
        or sample:find("520: web server",1,true)~=nil
...
local results={
    Meta={
        Version="NasiPayloadFetcherV3",
        PlaceId=game.PlaceId,
        JobId=game.JobId,
        Unix=os.time(),
        Note="Fetch-only. Remote text is never executed.",
    },
    Targets={}
}

local root=parentGui()
local old=root:FindFirstChild("PSICO_NASI_PAYLOAD_FETCHER")
...
frame.Size=UDim2.fromOffset(470,320)
...
title.Text="NASI PAYLOAD FETCHER V3"
...
sub.Text="FETCH-ONLY • nenhum payload remoto é executado"
sub.TextColor3=Color3.fromRGB(130,160,195)
sub.TextSize=10
...
status.Size=UDim2.new(1,-28,0,114)
...
status.Text="Novo alvo: OMAK 121.\nBaixe somente esse primeiro e depois exporte o resumo."
status.Parent=frame
...
local btn1=mk("OMAK 121",14,187,210)
local btn2=mk("NRL SAE CORE",232,187,210)
local btn3=mk("NRL LOADER",14,229,210)
local btn4=mk("JNKIE PUBLIC",232,229,210)
local btn5=mk("EXPORTAR RESUMO",14,271,428)
...
local function runTarget(t)
    ...
    if t.genvKeyless and getgenv then
        pcall(function() getgenv().SCRIPT_KEY="KEYLESS" end)
    end
    ...
    status.Text=string.format(
        ...
        t.name,tostring(httpStatus),#body,
        a.looksLua and "SIM" or "NAO",
        a.luraph and "SIM" or "NAO",
        a.cloudflare and "SIM" or "NAO",
        #a.keywords>0 and table.concat(a.keywords,", ") or "nenhuma",
        filename
    )
    busy=false
end
...
btn4.Activated:Connect(function() task.spawn(runTarget,TARGETS[4]) end)
btn5.Activated:Connect(function()
    local ok,json=pcall(Http.JSONEncode,Http,results)
    if not ok then
        status.Text="Erro ao gerar resumo."
        return
    end
...
local function resize()
    ...
    frame.Size=UDim2.fromOffset(
        math.clamp(vp.X*.68,350,470),
        math.clamp(vp.Y*.75,300,320)
    )
end
resize()
