--[[
PSICOSENATICO | NASI DIFFERENTIAL SNAPSHOT V1.2
Zero-hook. Custom JSON encoder to avoid HttpService JSON conversion failures.
Flow: CAPTURAR ANTES -> carregar Steal An Egg -> CAPTURAR DEPOIS -> EXPORTAR.
Optional ANALISE COMPLETA: 12s, sample every 2s, stores only diffs.
Also records a passive file-name manifest when listfiles is available.
]]
...
local CFG={MaxUI=1600,MaxGlobals=800,MaxRelevant=800,MaxAttrs=25,MaxTableKeys=25,MaxText=600,AutoSeconds=12,AutoInterval=2,MaxFiles=500}
...
local function sensitive(k)k=lower(k);return k:find("token",1,true) or k:find("cookie",1,true) or k:find("password",1,true) or k:find("auth",1,true) or k:find("session",1,true) or k:find("key",1,true) end
...
local function attrsOf(x)
 ...
 local n=0;for k,v in pairs(a) do n+=1;if n>CFG.MaxAttrs then break end;local t=typeof(v);if t=="string" or t=="number" or t=="boolean" then out[safe(k)]=scalar(v) end end;return out
end
local function prop(out,name,fn)local ok,v=pcall(fn);if ok then out[name]=scalar(v) end end
local function describeUI(x)
 local d={class=x.ClassName,name=cut(x.Name,160),attrs=attrsOf(x)}
 if x:IsA("GuiObject") then prop(d,"Visible",function()return x.Visible end);prop(d,"Position",function()return x.Position end);prop(d,"Size",function()return x.Size end);prop(d,"ZIndex",function()return x.ZIndex end) end
 if x:IsA("TextLabel") or x:IsA("TextButton") or x:IsA("TextBox") then prop(d,"Text",function()return x.Text end) end
 if x:IsA("ScreenGui") then prop(d,"Enabled",function()return x.Enabled end) end
 return d
end
...
local function captureFiles()
 ...
 for _,root in ipairs(roots) do
  ...
  if ok and type(list)=="table" then
   for _,p in ipairs(list) do if count>=CFG.MaxFiles then break end;local s=safe(p);if not seen[s] then seen[s]=true;count+=1;out.items[s]={name=s} end end
  end
...
local function cycleHints()
 ...
 for _,x in ipairs(list) do if names[x.Name] then out[x.Name]=out[x.Name] or {};if #out[x.Name]<10 then local r=describeRelevant(x);r.path=pathOf(x);out[x.Name][#out[x.Name]+1]=r end end end;return out
end
local function snapshot(label)return {label=label,t=now(),unix=os.time(),cycle=cycleHints(),ui=captureUI(),globals=captureGlobals(),relevant=captureRelevant(),files=captureFiles()} end
local function stable(v,seen)
 local t=type(v)
 if t~="table" then return safe(v) end
 seen=seen or {};if seen[v] then return "<cycle>" end;seen[v]=true
 local keys={};for k in pairs(v) do keys[#keys+1]=safe(k) end;table.sort(keys);local parts={}
 for _,ks in ipairs(keys) do local found=nil;for k,val in pairs(v) do if safe(k)==ks then found=val;break end end;parts[#parts+1]=ks.."="..stable(found,seen) end;seen[v]=nil;return "{"..table.concat(parts,"|").."}"
end
...
local function counts(d)return string.format("UI +%d ~%d -%d | G +%d ~%d | Rel +%d ~%d | Files +%d",c(d.ui.added),c(d.ui.changed),c(d.ui.removed),c(d.globals.added),c(d.globals.changed),c(d.relevant.added),c(d.relevant.changed),c(d.files.added)) end
...
local function report()return {Meta={Version="NasiDifferentialSnapshotV1.2",StartedUnix=START_UNIX,FinishedUnix=os.time(),PlaceId=game.PlaceId,GameId=game.GameId,JobId=game.JobId,ZeroHook=true,CustomJSON=true,AutoSeconds=CFG.AutoSeconds,AutoInterval=CFG.AutoInterval},Before=State.before,After=State.after,Comparison=State.compare,AutoRuns=State.autoRuns} end
local function jesc(s)
 s=safe(s);local out={"\""}
 for i=1,#s do local b=string.byte(s,i);if b==34 then out[#out+1]='\\\"' elseif b==92 then out[#out+1]='\\\\' elseif b==8 then out[#out+1]='\\b' elseif b==9 then out[#out+1]='\\t' elseif b==10 then out[#out+1]='\\n' elseif b==12 then out[#out+1]='\\f' elseif b==13 then out[#out+1]='\\r' elseif b>=32 and b<=126 then out[#out+1]=string.char(b) else out[#out+1]=string.format('\\u%04x',b) end end
...
local function encode(v,seen)
 ...
 for k,val in pairs(v) do out[#out+1]=jesc(safe(k))..":"..encode(val,seen) end;table.sort(out);seen[v]=nil;return "{"..table.concat(out,",").."}"
end
local function export()
 if not State.before and #State.autoRuns==0 then status("Nada para exportar.");return end
 local ok,json=pcall(encode,report());if not ok then status("Erro no encoder V1.2: "..cut(json,220));return end
 local name="Psico_Nasi_Differential_V1_2_"..os.time()..".json";local wrote=false;if type(writefile)=="function" then wrote=pcall(writefile,name,json) end;if type(setclipboard)=="function" then pcall(setclipboard,json) end;status((wrote and "EXPORTADO: " or "JSON COPIADO: ")..name.."\n"..#json.." bytes | Runs: "..#State.autoRuns)
end
