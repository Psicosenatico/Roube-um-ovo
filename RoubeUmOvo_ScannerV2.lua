--[[
PSICOSENATICO | Roube um Ovo - Egg Catalog Scanner V6.2
Stable loader path: RoubeUmOvo_ScannerV2.lua

V6.2 goals:
  * Recover eggs that already existed BEFORE the scanner started.
  * Do not depend on the next egg reset/rotation.
  * Sources: current visual folder, EggWorld snapshots, already-loaded client memory,
    loaded modules, replicated values and module constants.
  * Track only State == "Slot" as authoritative nest eggs.
  * Keep visual size signals separate from any official Weight/Mass fields.
  * Export only JSON-safe data.
]]
...
local function plain(v,depth,seen)
    ...
    if t=="string" then return #v>CONFIG.JsonString and (v:sub(1,CONFIG.JsonString).."<truncated>") or v end
    if t=="number" then
    ...
    if t=="table" then
        ...
        pcall(function()
            for k,val in pairs(v) do
                count+=1
                if count>120 then out["<truncated>"]=true break end
...
local function sanitize(v,depth,seen)
    ...
    local t=typeof(v)
    if t~="table" then return plain(v,0,{}) end
    if seen[v] then return "<cycle>" end
    seen[v]=true
    ...
    if dense then
        ...
        pcall(function()
            for k,val in pairs(v) do
                ...
                out[safeString(k)]=sanitize(val,depth+1,seen)
            end
        end)
    end
    seen[v]=nil
...
local function getField(t,names)
    ...
    pcall(function()
        ...
        for k,v in pairs(t) do
            ...
            if wanted[normalize(k)] then out=v break end
        end
    end)
    return out
end
...
local function mergeCatalog(identity,fields,source,path)
    ...
    for k,v in pairs(fields or {}) do
        ...
        if (nk:find("rarity",1,true) or nk=="tier" or nk=="grade") and type(v)=="string" and v~="" and #v<=50 then
...
local function inspectTree(root,source,evidence,rarityHint)
    ...
    local function walk(t,path,depth,fallback)
        ...
        if next(fields) then
            ...
            if not State.SeenEvidence[sig] then
                ...
                addLimited(evidence,{source=source,path=path,identity=id,fields=fields})
            end
        ...
        if rarityHint then
            pcall(function()
                local c=0
                for k,v in pairs(t) do
                    c+=1 if c>180 then break end
                    if type(v)=="string" and #v>0 and #v<=40 and normalize(k):find("rarity",1,true) then
                        State.RarityCandidates[v]=State.RarityCandidates[v] or {source=source,path=path,key=safeString(k)}
                    end
                end
            end)
        end
...
local function modelEvidence(uid)
    ...
    for _,d in ipairs(m:GetDescendants()) do
        ...
        elseif d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
            ...
            if text~="" and (containsWord(text,RELEVANT_KEYS) or text:find("%$") or lower(text):find("kg",1,true)) then
...
local function makeEgg(record,source)
    ...
    local egg={
        ...
        FirstSeen=old and old.FirstSeen or elapsed(),LastSeen=elapsed(),
    }
...
local function ingestRecord(record,source)
    ...
    if record.State=="Slot" then
        State.Eggs[record.Uid]=makeEgg(record,source)
        return true
    end
    return false
end
    ...
    for _,d in ipairs(ReplicatedStorage:GetDescendants()) do
        if d:IsA("RemoteEvent") then
            ...
            elseif d.Name=="RE/EggWorld/FieldEggBatchShifted" then
                connect(d.OnClientEvent,onBatch,State.RemoteConnections)
            ...
            elseif d.Name=="RE/EggWorld/FieldEggRedeemVerdict" then
                connect(d.OnClientEvent,onRedeem,State.RemoteConnections)
            end
        end
    end
end
...
local function refreshVisualFallback()
    ...
    for _,child in ipairs(folder:GetChildren()) do
        if child:IsA("Model") or child:IsA("BasePart") then
            ...
            elseif State.Eggs[uid].Source=="visual_folder" then
    ...
    for _,name in ipairs({"RF/EggWorld/AskFieldEggSnapshot","RF/EggWorld/AskLiveSnapshot"}) do
        ...
        if rf then
            ...
            if ok then
                ...
                addEvent("snapshot_error",{name=name,error=safeString(res)})
            end
        end
    end
    State.Stats.SnapshotRecords=total
    refreshStatus(total>0 and ("Snapshots: "..total.." ovos") or "Snapshots sem registros; lendo memória")
    return total
end

-- Critical V6.2 fix: recover Slot records that were already in memory before scanner startup.
local function scanMemoryForExistingEggs()
    ...
    for _,obj in ipairs(objects) do
        ...
        if typeof(obj)=="table" then
            local uid,state
            pcall(function()
                uid=rawget(obj,"Uid") or rawget(obj,"UID") or rawget(obj,"uid")
                state=rawget(obj,"State") or rawget(obj,"state")
            end)
    ...
    addEvent("memory_existing_eggs",{records=found,objectsScanned=scanned})
    refreshStatus(found>0 and ("Memória inicial: "..found.." ovos") or nil)
    return found
end
    ...
    for _,m in ipairs(list) do
        ...
        if typeof(m)=="Instance" and m:IsA("ModuleScript") then
            ...
            if containsWord(p,TOPIC_WORDS) then
                ...
                if rok and typeof(res)=="table" then inspectTree(res,"LoadedModule:"..p,State.LoadedModuleEvidence,lower(p):find("rarity",1,true)~=nil) end
            end
        end
    end
...
local function scanGcCatalog()
    ...
    for _,obj in ipairs(objects) do
        ...
        if typeof(obj)=="table" then
            ...
            if score>=3 then
                roots+=1
                inspectTree(obj,"GC:"..roots,State.GcEvidence,false)
            end
        end
    end
            ...
            if ok and typeof(fn)=="function" then
                ...
                if cok and type(consts)=="table" then
                    ...
                    for _,v in pairs(consts) do
                        if type(v)=="string" and #v>0 and #v<=100 and containsWord(v,RELEVANT_KEYS) then
                            table.insert(kept,v)
...
local function runScan()
    ...
    refreshStatus("Recuperando estado atual...")
    ...
    if ok then
        ...
        refreshStatus("Varredura concluída sem esperar reset")
    else
...
local function report()
    ...
    return {
        Version="Static Egg Catalog Scanner V6.2",
        Meta={PlaceId=game.PlaceId,GameId=game.GameId,JobId=game.JobId,StartedUnix=startedUnix,FinishedUnix=os.time(),DurationSeconds=elapsed(),InteractionRequired=false,Notes="Recovers pre-existing Slot eggs from snapshots, visual folder and client memory."},
        Stats=State.Stats,
...
local function exportReport()
    ...
    if not ok then return false,"JSONEncode falhou: "..safeString(encoded) end
    ...
    if writefile then
        ...
        return false,"writefile falhou: "..safeString(e)
    end
    if setclipboard then
        local cok,e=pcall(setclipboard,encoded)
        if cok then return true,"JSON copiado" end
        return false,"setclipboard falhou: "..safeString(e)
    end
...
task.defer(function()
    task.wait(.8)
    if State.Alive then runScan() end
end)

task.defer(function()
    while State.Alive do task.wait(1.5) pcall(refreshVisualFallback) end
end)
...
frame.AnchorPoint=Vector2.new(.5,.5) frame.Position=UDim2.fromScale(.5,.5) frame.Size=UDim2.fromOffset(306,218)
...
title.Font=Enum.Font.GothamBold title.Text="EGG CATALOG SCANNER • V6.2" title.TextColor3=Color3.fromRGB(241,245,255) title.TextSize=13 title.TextXAlignment=Enum.TextXAlignment.Left title.Parent=frame
...
statusLabel.Position=UDim2.fromOffset(12,43) statusLabel.Size=UDim2.new(1,-24,0,72) statusLabel.BackgroundColor3=Color3.fromRGB(21,29,44)
statusLabel.BorderSizePixel=0 statusLabel.Font=Enum.Font.Code statusLabel.TextColor3=Color3.fromRGB(174,198,241) statusLabel.TextSize=10 statusLabel.TextWrapped=true statusLabel.Parent=frame
Instance.new("UICorner",statusLabel).CornerRadius=UDim.new(0,9)
local rescan=Instance.new("TextButton")
rescan.Position=UDim2.fromOffset(12,125) rescan.Size=UDim2.new(.5,-17,0,36) rescan.BackgroundColor3=Color3.fromRGB(31,55,100) rescan.BorderSizePixel=0 rescan.Font=Enum.Font.GothamMedium rescan.Text="Revarrer memória" rescan.TextColor3=Color3.fromRGB(240,244,255) rescan.TextSize=11 rescan.Parent=frame
Instance.new("UICorner",rescan).CornerRadius=UDim.new(0,9)
local export=Instance.new("TextButton")
export.Position=UDim2.new(.5,5,0,125) export.Size=UDim2.new(.5,-17,0,36) export.BackgroundColor3=Color3.fromRGB(37,82,170) export.BorderSizePixel=0 export.Font=Enum.Font.GothamMedium export.Text="Exportar JSON" export.TextColor3=Color3.fromRGB(245,248,255) export.TextSize=11 export.Parent=frame
Instance.new("UICorner",export).CornerRadius=UDim.new(0,9)
local hint=Instance.new("TextLabel")
hint.BackgroundTransparency=1 hint.Position=UDim2.fromOffset(12,169) hint.Size=UDim2.new(1,-24,0,36) hint.Font=Enum.Font.Gotham
hint.Text="Não precisa esperar os ovos reiniciarem. Recupera o estado atual." hint.TextColor3=Color3.fromRGB(151,163,188) hint.TextSize=10 hint.TextWrapped=true hint.Parent=frame
...
refreshStatus("Recuperando ovos que já estavam no mapa...")